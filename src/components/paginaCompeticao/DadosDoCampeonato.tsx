        ...
        <div className="flex flex-col gap-4 lg:grid lg:grid-cols-2 xl:gap-10">
            {
                mapas && mapas.length > 0 ? (
                    <div className="lg:col-start-1 lg:col-end-2">
                        <h3 className="font-heading text-3xl">Mapa Mais Jogado: <span className="capitalize">{mapas[0].mapa != null ? mapas[0].mapa : 'Indisponível'}</span> - {mapas[0].quantidade}</h3>
                                    ...
                                    mapas.slice(1).map((mapa, i) => {
                                        return (
                                            <li key={i} className="relative">
                                                <div className="w-full h-[67px] relative rounded-t-md overflow-hidden">
            ...
            <div className="flex flex-col gap-4 lg:col-start-2 lg:col-end-4 lg:grid lg:grid-cols-2">
                    ...
                    <ul className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-3">
                        {
                            campeonato.premiacoes?.map((premiacao, i) => {
                                return (
                                    <li key={i}>
                                        {
                                            premiacao.classificacaoProximoStage ? (
                                                <p>Classificação para o Próximo Stage!</p>
                                            ) : (
                                                <p>{premiacao.colocacao}: <strong>{premiacao.valor}</strong> {premiacao.moeda}</p>
                                            )
                                        }
                                    </li>
                                )
